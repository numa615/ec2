...
if __name__ == "__main__":
  ...
  def mean_dist(input): #calculate the average distances                                                                                 
      output = []
      for item in input:
        if item == '':
          output.append('')
  ...
  placekey_filter = sc.textFile('/tmp/bdm/core-places-nyc.csv') \
          .map(lambda x: next(csv.reader([x]))) \
          .filter(lambda x:x[9][:4] == '4451').map(lambda x:[x[9],x[0]]) \
          .map(lambda x: x[1])\
          .collect()
        
  rdd_first = sc.textFile('/tmp/bdm/weekly-patterns-nyc-2019-2020') \
  ...
  result = rddT4 \
        .map(lambda x: [x[0],mean_dist(x[1])]) \
        .map(lambda x: ((str(x[0]), str(x[1][0]) , str(x[1][1]), str(x[1][2]), str(x[1][3]))))\
        .toDF(['cbg_fips', '2019-03' , '2019-10' , '2020-03' , '2020-10']) \
        .sort('cbg_fips', ascending = True) #calculate the average distance of each cbgs of each months and then transfer rdd to dataframe

  result.write.options(header = 'True').csv(sys.argv[1])
